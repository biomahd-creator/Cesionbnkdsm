        ...
        <table className="border-collapse">
          <thead>
            <tr>
              ...
              {columns.map((col) => (
                <th key={col} className="border border-border p-2 text-sm font-semibold bg-muted/30" style={{ width: `var(--heat-cell-size, ${cellSize}px)`, height: `var(--heat-cell-size, ${cellSize}px)` }}>
                  {showLabels ? col : ""}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row}>
                <td className="border border-border p-2 text-sm font-semibold bg-muted/30" style={{ width: `var(--heat-cell-size, ${cellSize}px)` }}>
                  {showLabels ? row : ""}
                </td>
                {columns.map((col) => {
                  ...
                  return (
                    <td
                      ...
                      style={{ "--heat-bg": bgColor, "--heat-color": textColor, "--heat-cell-size": `${cellSize}px`, width: `var(--heat-cell-size)`, height: `var(--heat-cell-size)` } as React.CSSProperties}
                      data-heat-bg={true}
