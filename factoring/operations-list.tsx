/**
 * operations-list.tsx — Gestión de Operaciones (C-Financia Admin)
 *
 * FLAT TABLE: Tabla plana mostrando todas las facturas directamente.
 * - Una fila por factura (sin niveles jerárquicos)
 * - Columnas: Checkbox | Nº Factura | Pagador | Cliente | Operación | Fecha Venc. | V. Factura | V. Desembolso | Estado Fact. | Estado Op. | Acciones
 * - Selección individual por factura
 * - Filtros por estado de operación y búsqueda global
 * - Siempre visible, sin expand/collapse
 *
 * v0.3.2 — Phase 5b: PascalCase backing file inlined; OperationsList.tsx eliminated.
 * v0.8.0 — Flat table: Tree structure removed, direct factura listing.
 */
...
import { FactoringKpiCardGroup } from "./components/factoring-kpi-card-group";
import { ReportsConsultation } from "../components/patterns/reports-consultation";
...
const mockOperations: Operation[] = [
  ...
  },
  {
    ...
    monto: 45000, plazo: 30, tasa: 2.5, comision: 1125, estado: "Endosada", subEstado: "Operación notificada",
...
export function OperationsList({ onNewOperation }: OperationsListProps = {}) {
  ...
  /* ── Data pipeline: Flatten to facturas ── */
  const flatFacturas = useMemo(() => {
    let ops = mockOperations;
    if (activeKpiFilter !== "all") ops = ops.filter((op) => op.estado === activeKpiFilter);
    if (estadoFilter !== "all") ops = ops.filter((op) => op.estado === estadoFilter);
    
    // Flatten to factura rows
    const facturas: Array<FacturaDetalle & { operacion: Operation }> = [];
    for (const op of ops) {
      for (const factura of op.facturas) {
        facturas.push({ ...factura, operacion: op });
      }
    }
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(f => 
        ...
        f.pagador.nombre.toLowerCase().includes(q) ||
        f.pagador.nit.includes(q) ||
        f.operacion.id.toLowerCase().includes(q) ||
        f.operacion.cliente.nombre.toLowerCase().includes(q) ||
        f.operacion.cliente.nit.includes(q)
      );
    }
    
    return result;
  }, [activeKpiFilter, estadoFilter, searchQuery]);

  const allFacturaIds = useMemo(() => flatFacturas.map(f => f.id), [flatFacturas]);

  /* ── Handlers ── */
  const toggleFacturaSelection = useCallback((facturaId: string) => {
    setSelectedIds((prev) => {
  ...
  const handleSelectAll = () => {
    ...
    else setSelectedIds(new Set(allFacturaIds));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden border-border shadow-lg bg-card">
        ...
        {/* ═══ SELECTION BAR ═══ */}
        {selectedIds.size > 0 && (
            <div className="px-4 py-2 bg-muted border-b flex items-center justify-between animate-in slide-in-from-top duration-300">
          ...
          <Table>
            <TableHeader className="bg-muted">
              <TableRow>
                ...
                <TableHead className="w-[130px]">Nº Factura</TableHead>
                <TableHead className="w-[180px]">Pagador</TableHead>
                <TableHead className="w-[180px]">Cliente</TableHead>
                <TableHead className="w-[120px]">Operación</TableHead>
                <TableHead className="w-[110px]">Fecha Venc.</TableHead>
                <TableHead className="w-[130px]">V. Factura</TableHead>
                <TableHead className="w-[130px]">V. Desembolso</TableHead>
                <TableHead className="w-[110px] text-center">Estado Fact.</TableHead>
                <TableHead className="w-[110px] text-center">Estado Op.</TableHead>
                <TableHead className="w-[100px] text-center">Acciones</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
            {flatFacturas.length > 0 ? (
                flatFacturas.map((factura) => {
                    const isSelected = selectedIds.has(factura.id);
                    const facturaMatch = searchQuery.trim() && (
                      factura.numero.toLowerCase().includes(searchQuery.toLowerCase()) ||
                      factura.pagador.nombre.toLowerCase().includes(searchQuery.toLowerCase())
                    );
                    const factEstadoCfg = estadoFacturaConfig[factura.estado];
                    const opEstadoCfg = estadoOperacionConfig[factura.operacion.estado] || { 
                      label: factura.operacion.estado, 
                      variant: "neutral-soft-outline" as const 
                    };
                    ...
                    return (
                        <TableRow
                            ...
                            className={cn(
                                ...
                                facturaMatch && "bg-yellow-50 dark:bg-yellow-950"
                            )}
                        ...
                        >
                            <TableCell onClick={(e) => e.stopPropagation()}>
                                <Checkbox
                                    checked={isSelected}
                                    onCheckedChange={() => toggleFacturaSelection(factura.id)}
                            ...
                            <TableCell>
                                <div className="flex flex-col">
                                    <span className="truncate max-w-[160px] text-sm font-medium">{factura.pagador.nombre}</span>
                                    <span className="text-[10px] text-muted-foreground">{factura.pagador.nit}</span>
                                </div>
                            </TableCell>
                            <TableCell>
                                <div className="flex flex-col">
                                    <span className="truncate max-w-[160px] text-sm font-semibold">{factura.operacion.cliente.nombre}</span>
                                    <span className="text-[10px] text-muted-foreground">{factura.operacion.cliente.nit}</span>
                                </div>
                            </TableCell>
                            <TableCell>
                                <span className="text-[10px] text-muted-foreground font-satoshi">
                                    {factura.operacion.id}
                                </span>
                            </TableCell>
                            <TableCell className="text-sm font-satoshi text-muted-foreground">
                                {factura.fechaVencimiento}
                            </TableCell>
                            <TableCell className="text-sm font-bold font-satoshi">
                                {formatCurrency(factura.valor)}
                            </TableCell>
                            <TableCell className="text-sm font-bold text-primary font-satoshi">
                                {formatCurrency(factura.valorDesembolso)}
                            </TableCell>
                            <TableCell className="text-center">
                                <Badge variant={factEstadoCfg.variant} className="text-[10px] px-2 py-0.5 font-bold uppercase">
                                    {factEstadoCfg.label}
                                </Badge>
                            </TableCell>
                            <TableCell className="text-center">
                                <Badge variant={opEstadoCfg.variant as any} className="text-[10px] px-2 py-0 h-5 font-bold uppercase tracking-tighter">
                                    {opEstadoCfg.label}
                                </Badge>
                            </TableCell>
                            <TableCell onClick={(e) => e.stopPropagation()}>
                                <div className="flex items-center justify-center gap-1">
                                    <Button 
                                      ...
                                      onClick={() => handleAction("view", factura.operacion.id)}
                                    >
                                        <Eye className="h-4 w-4" />
                                    </Button>
                                    {factura.operacion.estado === "En Proceso" && (
                                        <Button 
                                          variant="ghost" 
                                          size="sm" 
                                          className="h-7 w-7 p-0 text-green-600 hover:text-green-600 hover:bg-green-50" 
                                          onClick={() => handleAction("approve", factura.operacion.id)}
                                        >
                                            <CheckCircle className="h-4 w-4" />
                                        </Button>
                                    )}
                                    {factura.operacion.estado === "Negociada" && (
                                        <Button 
                                          variant="ghost" 
                                          size="sm" 
                                          className="h-7 w-7 p-0 text-blue-600 hover:text-blue-600 hover:bg-blue-50" 
                                          onClick={() => handleAction("endorse", factura.operacion.id)}
                                        >
                                    ...
                                    <Button 
                                      ...
                                      onClick={() => handleAction("cancel", factura.operacion.id)} 
                                      disabled={factura.operacion.estado === "Liquidada" || factura.operacion.estado === "Endosada"}
                                    >
      ...
      {selectedOpDetail && (
        <div className="fixed inset-0 z-[100] bg-background backdrop-blur-sm flex items-center justify-end animate-in fade-in duration-300">
                ...
                <div className="grid gap-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid gap-3">
                            {[1, 2].map((i) => (
                                <div key={i} className="flex items-center justify-between p-3 rounded-lg border bg-muted">
                                    <div className="flex items-center gap-3">
