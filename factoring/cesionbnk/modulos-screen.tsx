...
export function ModulosScreen({ onSelectModule, onLogout }: ModulosScreenProps) {
  return (
    <div
      className="min-h-screen w-full overflow-hidden relative bg-background"
    >
          ...
          <div className="flex flex-col gap-4 w-full max-w-[520px]">
            {modules.map((mod) => {
              ...
              return (
                <Card
                  key={mod.id}
                  className="group cursor-pointer border-border transition-all duration-300 hover:border-primary hover:shadow-lg hover:scale-[1.02]"
                  onClick={() => onSelectModule(mod.id)}
                >
                  <CardContent className="p-6 flex items-center gap-5">
                    {/* Icon */}
                    <div className="shrink-0 flex h-14 w-14 items-center justify-center rounded-xl bg-muted text-primary transition-all duration-300 group-hover:bg-muted">
