  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card className="elevation-2 border-2 border-primary bg-card">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-muted-foreground">Cupo Disponible</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              <p className="text-3xl font-bold text-primary">{formatCOP(cupoDisponible)}</p>
              ...
              <p className="text-xs text-muted-foreground">de {formatCOP(cupoTotal)} total</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="elevation-2">
          ...
          <CardContent>
            <div className="space-y-2">
              <p className="text-3xl font-bold text-green-600 dark:text-green-400">{formatCOP(liquidezMesActual)}</p>
              <div className="flex items-center gap-1 text-sm">
                <ArrowUpRight className="h-4 w-4 text-green-500" />
        ...
        <Card className="elevation-2">
          ...
          <CardContent>
            <div className="space-y-2">
              <p className="text-3xl font-bold text-orange-600 dark:text-orange-400">{operacionesEnProceso}</p>
              <div className="flex items-center gap-1 text-sm">
          ...
          <CardContent>
            <SafeChartContainer width="100%" height="300px">
              <AreaChart data={liquidezMensual}>
                ...
                />
                <Legend />
                <Area type="monotone" dataKey="liquidez" name="Liquidez Obtenida" stroke="#84cc16" fillOpacity={1} fill="url(#colorLiquidez)" />
                <Area type="monotone" dataKey="comisiones" name="Comisiones Pagadas" stroke="#ef4444" fillOpacity={1} fill="url(#colorComisiones)" />
              </AreaChart>
             </SafeChartContainer>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="flex items-center gap-6">
              <SafeChartContainer width="100%" height="250px" className="w-[50%]">
                <PieChart>
                  <Pie data={pieDataEstados} cx="50%" cy="50%" innerRadius={60} outerRadius={90} dataKey="cantidad" paddingAngle={2}>
        ...
        <CardContent>
          <div className="space-y-3">
            {proximosVencimientos.map((item, index) => (
              <div key={index} className="flex items-center justify-between p-4 rounded-lg border bg-card hover:bg-muted transition-colors">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="font-semibold">{item.cliente}</p>
                    <Badge variant="info-soft-outline" className="text-xs">{item.factura}</Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">Monto: {formatCOPFull(item.monto)}</p>
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <p className={`text-sm font-semibold ${item.estado === 'urgente' ? 'text-destructive' : 'text-muted-foreground'}`}>{item.dias} días</p>
          ...
          <div className="space-y-3">
            {operacionesRecientes.map((item, index) => (
              <div key={index} className="flex items-center justify-between p-4 rounded-lg border bg-card">
                <div className="flex items-center gap-4">
                  <div className={`p-2 rounded-lg ${item.estado === 'completado' ? 'bg-green-50 dark:bg-green-950' : 'bg-orange-50 dark:bg-orange-950'}`}>
                ...
                <div className="text-right">
                  <p className="font-semibold text-lg">{formatCOPFull(item.monto)}</p>
                  <Badge variant={item.estado === 'completado' ? 'success-soft-outline' : 'warning-soft-outline'}>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="elevation-1">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-muted-foreground">Comisiones Acumuladas (Mes)</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              <p className="text-2xl font-bold text-destructive">{formatCOPFull(comisionesAcumuladas)}</p>
              <div className="flex items-center gap-1 text-sm">
                <Percent className="h-4 w-4 text-muted-foreground" />
                <span className="text-muted-foreground text-xs">{((comisionesAcumuladas / liquidezMesActual) * 100).toFixed(2)}% del total</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="elevation-1">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-muted-foreground">Tiempo Promedio de Aprobación</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">4.2 horas</p>
              <div className="flex items-center gap-1 text-sm">
        ...
        <Card className="elevation-1">
          ...
          <CardContent>
            <div className="space-y-2">
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">94.5%</p>
              <div className="flex items-center gap-1 text-sm">
